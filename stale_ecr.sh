...
# Function to fetch CloudTrail events
fetch_cloudtrail_events() {
    ...
    # Fetch CloudTrail events for ECR operations
    # We look for BatchGetImage (actual image pulls) and GetDownloadUrlForLayer (layer downloads)
    aws logs start-query $AWS_OPTS \
        ...
        | sort @timestamp desc' 2>/dev/null || {
        
        # Fallback to CloudTrail API if CloudWatch Logs not available
        echo "CloudWatch Logs approach failed, trying CloudTrail API..."
    ...
    if [[ -s "$temp_file" ]]; then
        ...
        rm -f "$temp_file"
        echo -e "${GREEN}CloudTrail events cached to: $CLOUDTRAIL_CACHE${NC}"
    else
        echo -e "${RED}No CloudTrail events found. Ensure CloudTrail is enabled for ECR API calls.${NC}"
        exit 1
    fi
}
